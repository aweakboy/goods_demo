<template>
  <div class="page-container">
    ...
    <el-card v-for="order in orders" :key="order.id" class="order-card">
      ...
      <p style="color:#666;margin:8px 0">收货地址：{{ order.address }}</p>
...
const shipForm = ref({ orderId: null, trackingNumber: '' })
...
function openShipDialog(order) {
  shipForm.value = { orderId: order.id, trackingNumber: '' }
...
async function doShip() {
  ...
  try {
    await orderApi.ship(shipForm.value.orderId, shipForm.value.trackingNumber)
